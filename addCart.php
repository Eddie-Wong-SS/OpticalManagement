<?php
/**
 Allows the filtering of items to add to the cart
 */
...
<div class="container">
    ...
    if($_REQUEST['btnSearch'])
    {
        ...
        $SQL = "SELECT * FROM tblinventory, tblsellprice, tblcurrentstock";
        $SQL .= " WHERE ";
        ...
        $SQL .= "AND tblinventory.Status = 'A' AND 
        tblinventory.ItemId = tblsellprice.ItemId AND tblinventory.ItemId = tblcurrentstock.ItemId AND tblcurrentstock.CurQuan > tblinventory.ReorderLim";
        $SQl .= "ORDER BY ItemType";
        $SQL = str_replace("WHERE AND", "WHERE", $SQL);
        $SQL = str_replace("WHERE ORDER","ORDER", $SQL);
        $_SESSION['SQL'] = $SQL;
        $id = $_GET['id'];
        echo "<script>location='addCartResult.php?id=".$id."&page=1'</script>";
    }
    ?>
</div>
</body>